...
window.Layout = (() => {
    ...
    const TOPBAR_LINKS = [
        { name: 'Manifesto', url: 'manifesto.html' },
    ...
    function _injectTopBar() {
        ...
        TOPBAR_LINKS.forEach(link => {
            const isActive = link.url === decodedPath ||
                (decodedPath === '' && link.url === 'manifesto.html') ||
                (decodedPath === 'index.html' && link.url === 'manifesto.html');
            html += `<a href="${link.url}" class="top-bar-link ${isActive ? 'active' : ''}">${link.name}</a>`;
        });
        html += '</div>';
    ...
    return { init, renderNavItems, renderHero };
})();
